...
- [x] `src/game/SaveManager.ts` 実装
  - 仕様参照: `docs/functional-design.md` § セーブデータ
  - 完了条件: ゲーム進捗をLocalStorageに保存・読み込みできる
- [x] TitleScene に「つづきから」機能を組み込む
  - 完了条件: セーブデータがある場合、途中から再開できる
- [x] 全体通しプレイ・バグ修正
  - 完了条件: Realm1〜9まで通しでプレイできる
- [x] **QA: 仕様照合・バグ確認**
  - 2026-03-24 QA実施・全指摘修正完了
  - 修正内容: 敗北後HP回復、装備ボタン、Realm9エンディング遷移、ボスセリフ、装備ドロップ、雑魚/ボス区別、SaveManager統一、固定マスバグ修正、全マス不正解メッセージ
...
| Phase 4: ロジック統合 | ✅ 完了 |
| Phase 5: セーブ・ロード・仕上げ | ✅ 完了 (2026-03-24) |
